...
class ScoutScheduler:
    ...
    def __init__(
        ...
        self.match_assignments = {match: {
            name: 0 for name in scout_names} for match in range(1, total_matches + 1)}
    ...
    def select_scout_for_team(self, match: int, team: str, available_scouts: List[str]) -> str:
        ...
        for scout in eligible:
            ...
            score = (
                ...
                # Negative because lower is better (prefer continuity)
                -continuity,
                total_assignments,     # Balance total workload across scouts
                # Negative because lower is better (prefer familiarity)
                -familiarity
            )
    ...
    def update_tracking_for_match(self, match: int) -> None:
        ...
        for scout in self.scout_names:
            ...
            worked = any(self.schedule[match][team] ==
                         scout for team in self.teams_to_scout)
        ...
        for scout in self.scout_names:
            ...
            print(
                f"  {scout}: {status} | Streak: {self.active_streaks[scout]} | Last: {self.last_worked[scout]}")
        ...
        for segment_start, segment_end in self.segments:
            ...
            for match in range(segment_start, segment_end + 1):
                ...
                if len(available_scouts) < len(self.teams_to_scout):
                    raise ValueError(
                        f"Not enough scouts available for match {match}")
                ...
                for team in self.teams_to_scout:
                    ...
                    selected_scout = self.select_scout_for_team(
                        match, team, available_scouts)

                    # Update schedule
                    self.schedule[match][team] = selected_scout
                    # Update scout assignments
                    self.scout_assignments[selected_scout].append(
                        (match, team))
                    ...
                    # Adjust if multiple assignments per match are allowed
                    if self.match_assignments[match][selected_scout] >= 1:
                        available_scouts = [
                            s for s in available_scouts if s != selected_scout]
    ...
    def visualize_schedule(self, filename: str = fun + "scout_schedule.png") -> None:
        ...
        numerical_data = np.zeros(
            (self.total_matches, len(self.teams_to_scout)))
        ...
        # Add break markers
        for break_match in self.breaks:
            ax.axhline(y=break_match - 0.5, color='red',
                       linestyle='-', linewidth=2, alpha=0.7)
            ax.text(-0.5, break_match, f"Break", color='red',
        ...
        # Add a legend
        handles = [plt.Rectangle((0, 0), 1, 1, color=cmap(
            scout_to_index[name])) for name in unique_scouts]
        plt.legend(handles, unique_scouts,
    ...
    def check_insufficient_rest(self, min_rest_period: int = None) -> Dict[str, List[Tuple[int, int, int]]]:
        ...
        last_team_assignment = {
            name: {team: 0 for team in self.teams_to_scout} for name in self.scout_names}
            ...
            for scout in working_scouts:
                ...
                if needs_rest[scout]:
                    ...
                    if rest_period < min_rest_period:
                        ...
                        is_break_between = any(
                            rest_started[scout] <= b < match for b in self.breaks)

                        if not is_break_between:
                            reason = "Replaced on a team" if consecutive_worked[
                                scout] < self.target_consecutive_matches else "Exceeded consecutive matches"
                            insufficient_rest[scout].append(
                                (rest_started[scout] - 1, match, rest_period, reason))
    ...
    def print_insufficient_rest_report(self, min_rest_period: int = None) -> None:
        ...
        for scout, violations in sorted(insufficient_rest.items(), key=lambda x: len(x[1]), reverse=True):
            for last_match, next_match, rest_period, reason in violations:
                print(
                    f"{scout:<15} {last_match:<15} {next_match:<15} {rest_period:<15} {min_rest_period:<15} {reason:<25}")
                total_violations += 1
        ...
        print(
            f"Scouts affected: {len(insufficient_rest)}/{len(self.scout_names)}")

        # Calculate the most common violations
        if total_violations > 0:
            violation_counts = Counter(
                [v[2] for scout_list in insufficient_rest.values() for v in scout_list])
            most_common = violation_counts.most_common(3)

            print("\nMost common insufficient rest periods:")
            for rest_period, count in most_common:
                print(
                    f"  {rest_period} matches: {count} occurrences ({count/total_violations*100:.1f}%)")

            # Count violations by reason
            reason_counts = Counter(
                [v[3] for scout_list in insufficient_rest.values() for v in scout_list])

            print("\nViolations by reason:")
            for reason, count in reason_counts.items():
                print(
                    f"  {reason}: {count} occurrences ({count/total_violations*100:.1f}%)")
...
def run_scout_scheduling():
    ...
    # Input data
    scout_names = ['HunterMadden', 'EmmaTran-Lee',
                   'GabrielleCarmy', 'DemirGoksu',
                   'ChaseMayer', 'QuinnHarris',
                   'JakeEffress', 'ZidaanSattar',
                   'AlexGavin', 'asherbutan',
                   'FeliciaDuan', 'VikramWright',
                   'SeanOzalpasan', 'AlexSegor',
                   'ClaudiaWiltshire', 'MatthewRen',
                   'MorganMaynard', 'ThomasConrad',
                   'MichaelBarr', 'IsabelErlic',
    ...
    unavailability = {
        # 'FeliciaDuan': [1, 2, 3],
        # 'VikramWright': [4, 5],
        'YariMilakin': [56, 57, 58, 59, 60, 61, 62, 63, 64, 65, 66, 67, 68, 69, 70, 71, 72, 73, 74],
        'NikoMa': [56, 57, 58, 59, 60, 61, 62, 63, 64, 65, 66, 67, 68, 69, 70, 71, 72, 73]
    }
    breaks = [22, 55]
